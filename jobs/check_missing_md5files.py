...
def missing_md5_list(storage, wkdir):
    ...
    # creating a nested dictionary containing runfolder as keys, bam/md5 and associated list of filenames as values in a nested dictionary
    ...
    # if bam filename in md5 filename list, then we know there is a matching md5 with the bam, create two lists one bam with md5, one bam without md5.
    for runfolder in runfolder_list:
        ...
        for bam_file in bam_files:
            ...
            if bam_md5 in bam_md5_dict[runfolder]["md5"]:
                ...
                with open(os.path.join(wkdir,'bam_with_md5.txt'), 'a') as good_bam:
                    ...
                    good_bam.writelines('md5 for {} is present \n'.format(bam_file))
            else:
                ...
                with open(os.path.join(wkdir,'bam_err_list.txt'), 'a') as bad_bam:
                    bam_file = os.path.abspath(runfolder) + "/" + bam_file
                    bad_bam.writelines('md5 for {} is missing \n'.format(bam_file))
    ...
    # if md5 filename not in bam filename list, create a list od md5, where the bam is missing.
        md5_files = bam_md5_dict[runfolder]["md5"]
        for md5_file in md5_files:
            md5_file = md5_file.split(".")
            md5_bam = md5_file[0] + "." + md5_file[1]
