...
# check_dict or org_bkup_check_dict() = dictionary created from check_md5() or create_check_dict()
...
def check_md5_hash(checkfilepath, md5, checksum):
    ...
    if len(checksum) == 32 and checksum.isalnum():
        print ("OK, {}: {} have 32 characters and contain only letters or numbers".format(md5, checksum))
        ...
        print ("ERROR, {}: {} DO NOT have 32 characters or contain non-alphanumeric characters".format(md5, checksum))
        #with open (checkfilepath, "a") as md5_check:
            #md5_check.writelines("time of hash check: {}\n".format(datetime.datetime.now()))
        ...
        if md5_filename not in check_dict:
            ...
            with open (md5, "r") as md5_file:
                ...
                if len(fields) == 2:
                    ...
                    change_rf_path = "cd {}".format(rf_path)
                    print (change_rf_path)
                    chg_rf_output = subprocess.call(change_rf_path, shell=True)
                    md5chk = "md5sum -c {}".format(md5_filename)
                    print (md5chk)
...
def compare_md5hash(md5_filename, check_dict):
    ...
    if check_dict["storage"][md5_filename]:
        originalhash = check_dict["storage"][md5_filename]
    if check_dict["archive"][md5_filename]:
        backup_hash = check_dict["archive"][md5_filename]

    if backup_hash == originalhash:
        return True
    else:
        return False
...
def check_hash_exist(md5_filename, check_dict1, check_dict2):
    ...
    if check_dict1.get(md5_filename) in check_dict2.values():
        return True
...
def check_org_bkup(org_rf_path, bkup_rf_path, org_bkup_check_dict):
    ...
    for md5_filename in org_bkup_check_dict["archive"].items():
        print ("md5_filename")

        if md5_filename in org_bkup_check_dict["archive"].items():
            backup_hash = org_bkup_check_dict["archive"][md5_filename]
        else:
            backup_hash == ""

        if md5_filename in org_bkup_check_dict["storage"].items():
