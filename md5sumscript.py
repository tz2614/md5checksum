...
def check_md5(checkfilepath, check_list):
    ...
    for md5 in check_list:
        ...
        """using md5sum -c, read md5 checksums from .md5 files and check if they match those generated by the md5sum program"""

        with open (checkfilepath, 'a') as md5_check:
            md5_check.writelines("\nmd5sum check:\n")

        md5chk = "cd {}; md5sum -c {} >> {} 2>> {}".format(rf_path, md5_filename, checkfilepath, checkfilepath)
        chk_output = subprocess.call(md5chk, shell=True)
        
        """check the bam filename associated with the md5, and the md5 checksum using the check_filename and check_md5_hash functions"""
...
def check_md5filename(rf_path, md5_filename, check_dict):
    ...
    """check if md5 file name is present in check_dict dictionary"""
...
def check_org_bkup(org_rf_path, bkup_rf_path, org_checkfilepath, bkup_checkfilepath, org_check_dict, bkup_check_dict):
    ...
    """Start the cross check between backup and original runfolder"""

    print ("BACKUP vs ORIGINAL\n")
    with open (bkup_checkfilepath, "a") as bkup_check:
        bkup_check.writelines("\nBACKUP vs ORIGINAL\n\n")
        bkup_check.writelines("time of check: {}\n".format(datetime.datetime.now()))

    """iterate through the dictionary and see if md5 filenames and checksums in the backup dicionary match those in the original runfolder"""
    ...
    """Start the cross check between original and backup runfolder"""
    print
    print ("ORIGINAL vs BACKUP\n")
    with open (org_checkfilepath, "a") as org_check:
        org_check.writelines("\nORIGINAL vs BACKUP\n\n")
        org_check.writelines("time of check: {}\n".format(datetime.datetime.now()))

    """iterate through the dictionary and see if md5 filenames and checksums in the original dicionary match those in the backup runfolder"""
    ...
    """Print all four nested dictionaries on terminal"""
    print
...
def main(org_rf_path, bkup_rf_path):
    ...
    print ("creating log file for original runfolder")
    org_checkfilepath = create_logfile(org_rf_path)
    print ("creating log file for backup runfolder")
    ...
    #record the time taken for the check in the md5_check.txt log
    with open(org_checkfilepath, 'a') as time_text:
        time_text.writelines("\ntime taken to check bam files integrity in {}\n and {}: {} \n\n".format(org_rf_path, bkup_rf_path, timetaken))
