...
def create_bam_md5_dict(runfolders, wkdir):
    ...
    assert runfolders.startswith("/mnt/storage/data/NGS") 
        or runfolders.startswith("/mnt/archive/data/NGS"), "runfolder given incorrect"

    pattern = "{}/[0-9][0-9][0-9][0-9][0-9][0-9]_*/bams/*".format(str(runfolders))
    
    #generate a list of filepath to the runfolders containing bam, md5 files in the geminifolder. 

    runfolder_list = list(sorted(set(glob.glob(pattern))))
...
#specify the directory where the list of files missing md5 will be recorded - sys.argv[2]
